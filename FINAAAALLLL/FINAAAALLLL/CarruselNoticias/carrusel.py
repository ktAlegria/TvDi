...
#para enviar la aplicacion por el carrusel
ait1_pid = 2001
dsmccB_association_tag = 0xB
dsmccB_carousel_id = 1

version = 104 #cambiar siempre que se haga modificaciones gaalvarez
...
nit = network_information_section(
	network_id = 1,
	    ...
	    network_descriptor(network_name = "Avalpa p",), 
            ],
...
sdt = service_description_section(
	...
	original_network_id = avalpa_original_transport_stream_id,
		...
		service_descriptor_loop = [
			...
			service_provider_name = "Avalpa p",
			service_name = "Avalpa prueba",
		    ),    
		],
	    ),	
        ],
	...
	stream_loop = [
		...
		stream_loop_item(
			...
			element_info_descriptor_loop = [ 
				...
				AIT_version = 2,  # current ait version
			    ),
			]	
		),
		stream_loop_item(
...
ait = application_information_section(
		...
		# Some required application descriptor follows application id
			...
			application_name_descriptor(application_name = "Carrusel de Noticias"),
			dvb_j_application_descriptor(parameters = [""]), # parameter passed to the xlet
			dvb_j_application_location_descriptor(
				...
				initial_class = "org.university.unicauca.tdi.app.AppXlet",  # the starting class implementing Xlet interface
			),
        	]
        	),
   	],
